...
1. Standard Pytorch interface to the external Spyre Communication Library for accelerated Spyre-to-Spyre communication
2. Support the [`torch.distributed` interface](https://pytorch.org/docs/stable/distributed.html) through a custom [`Backend` module](https://github.com/pytorch/pytorch/blob/main/torch/csrc/distributed/c10d/Backend.hpp#L165)
3. Support the [functional collectives](https://github.com/pytorch/pytorch/blob/main/torch/distributed/_functional_collectives.py) through the fallback mechanism to `torch.distributed` from inductor
4. Support existing Pytorch out-of-the-box Pytorch consumers using the communication library provided by Pytorch

## **Proposed Implementation**

### External Spyre Communication Library

The Spyre Communication Library (Spyre Comms) is an external library that support efficient Spyre-to-Spyre device communication for both the point-to-point (P2P) and collective communication patterns. This library provides a stable C++ API for a Pytorch distributed functionality to call into. The Spyre Comms library assumes that all input tensors are already resident in Spyre device memory, and output tensor will remain in Spyre device memory.

The Spyre Comms only supports on-node (single server) communication. Multi-node support may be added in the future.
...
We plan to support functional collectives via the same fallback into `torch.distributed` when using the compiled execution mode with the Spyre enhanced inductor-based backend. In the future, we may choose to lower these collectives to the Spyre Comms library directly.
