...
Usage will involve adding `ProfilerActivity.PrivateUse1` in profile activites along with `ProfilerActivity.CPU` for CPU events.
...
#### Calculate peak utilization across all allocations
```
  usage_by_node = [allocations for allocations in allocator.lx_usage_hist]

  if usage_by_node:
      # Peak utilization
      max_used = max(
          sum(rec["size"] for rec in allocations)
          for allocations in usage_by_node
      )
      peak_util = max_used / allocator.limit
      print(f"Peak Utilization: {peak_util * 100:.2f}%")
      ...
      avg_util = avg_used / allocator.limit
      print(f"Average Utilization: {avg_util * 100:.2f}%")
```
