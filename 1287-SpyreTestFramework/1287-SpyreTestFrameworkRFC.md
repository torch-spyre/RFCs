...
#### Dynamic op, dtype, and module markers

In addition to YAML-defined tags, the framework automatically attaches pytest markers
derived from the op name, dtype, and module name for every test variant generated by
`@ops` and `@modules` which the upstream pytorch framework lacks. 
These markers require no YAML configuration -- they are added
...
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "op__torch_nn_functional_embedding"
...
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "op__torch_add"

# Run all float16 variants across all ops
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "dtype__float16"

# Combine op and dtype filters
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "op__torch_add and dtype__float16"

# Combine with YAML-defined tags
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "gpt-oss-20b and op__torch_nn_functional_embedding"

# Run all variants for a specific module
bash run_test.sh configs/example_test_config.yaml -v -m "module__nn_BatchNorm2d"

# Run a specific module with a specific dtype
bash run_test.sh configs/example_test_config.yaml -v -m "module__nn_Linear and dtype__float32"
...
### Dynamic marker selection (reference)

```bash
# Filter by op
pytest test_model_ops_v2.py -m "op__torch_nn_functional_embedding"

# Filter by dtype
pytest test_model_ops_v2.py -m "dtype__float16"

# Filter by module
pytest test_modules.py -m "module__nn_BatchNorm2d"

# Combine op + dtype
pytest test_model_ops_v2.py -m "op__torch_add and dtype__float16"
