...
#### Dynamic op and dtype markers

In addition to YAML-defined tags, the framework automatically attaches pytest markers
derived from the op name and dtype for every test variant generated by `@ops` which lacks in upstream pytorch test framework.
These markers require no YAML configuration — they are added dynamically at collection time.

Marker naming convention:
- Op name: `torch.nn.functional.embedding` -> `torch_nn_functional_embedding`
- Dtype: `torch.float16` -> `float16`

This enables filtering by specific op or dtype without modifying the YAML config:

```bash
# Run all variants for a specific op
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_nn_functional_embedding"

# Run all variants for a specific op (collect only, no execution)
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_nn_functional_embedding" --collect-only

# Run all variants for another op
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_add"

# Run all float16 variants across all ops
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "float16"

# Combine op and dtype filters
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_add and float16"

# Combine with YAML-defined tags
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "gpt-oss-20b and torch_nn_functional_embedding"
```

Note: Dots in op names are replaced with underscores since pytest markers do not
support dots. The mapping is 1:1 — `torch_nn_functional_embedding` always refers
to `torch.nn.functional.embedding`.
...
```bash
# Run only tests tagged with a specific model (YAML-defined tag)
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "model_name_depending_on_this_test_1"

# Run tests matching a name pattern
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -k "test_scalar_support"

# Run all variants for a specific op (dynamic marker, no YAML change needed)
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_nn_functional_embedding"

# Run all float16 variants for a specific op
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_add and float16"

# Dry-run: collect matching tests without executing
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_nn_functional_embedding" --collect-only
bash run_test.sh configs/gpt_oss_20b_spyre.yaml -v -m "torch_add" --collect-only
```
...
- Dynamic pytest markers for op name and dtype, enabling `pytest -m "torch_add"` and
  `pytest -m "float16"` filtering without any YAML configuratio
